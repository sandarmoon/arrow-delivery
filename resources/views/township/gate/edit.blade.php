  ...
  <main class="app-content">
    <div class="app-title">
      ...
      <ul class="app-breadcrumb breadcrumb">
        ...
        <li class="breadcrumb-item"><a href="{{route('gatetownships.index')}}">{{ __("Townships")}}</a></li>
      </ul>
    </div>
    <div class="row">
      <div class="col-md-12">
        <div class="tile">
          ...
          <form action="{{route('gatetownships.update',$township->id)}}" method="POST">
            @csrf
            @method('put')
            {{-- <div class="row my-3">
              <div class="col-4">
            ...
            <div class="form-group">
              ...
              <select class="form-control" id="city" name="gate">
                <option>Choose Gate</option>
                @foreach($gates as $row)
                <option value="{{$row->id}}" @if($township->sender_gate_id==$row->id) selected @endif>{{$row->name}}</option>
                @endforeach
              </select>
            </div>
